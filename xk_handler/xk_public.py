...
class PublicAPIHandler(BaseHandler):
    def reload_dhcp(self,file,force=False):
        ...
        if d['xk_dhcp_status'] != 'yes':
            ...
            #return 5 # Stopping DHCP
            return 2 # Returns a success status code
        if force is False:
            check_md5 = self.get_md5(file)
            if check_md5 != d['xk_dhcp_conf_md5']:
                return 1 # MD5 checksum failed
...
dhcp-option=15,%s\n''' % (d['xk_dhcp_pool_start'],d['xk_dhcp_pool_stop'],d['xk_dhcp_pool_netmask'],d['xk_dhcp_pool_lease'],d['xk_dhcp_pool_gw'],d['xk_dhcp_pool_dns1'],d['xk_dhcp_pool_dns2'],d['xk_dhcp_pool_domain'])
        ...
        try:
            ...
            return 4 # Write configuration Failed
        finally:
            f.close()
        new_md5 = self.get_md5(file)
        #print new_md5
        ...
        sv_rt = os.system("/etc/init.d/dnsmasq restart")
        if sv_rt == 0:
            return 2 # Write file is successfully，Reload Configuration Succeeded
        else:
            return 3 # Failed to restart services
    ...
    def get(self):
        ...
        if module == "record":
            ...
            if fun == "ch_status":
                ...
                # Gets the configuration file for the domain name
                domain = self.db.get("select * from xk_domain where id = %s",id)
                file = domain['file']
                file_md5 = domain['file_md5']
                # Delete the DNSmasq configuration file for the domain name
                os.remove("/etc/dnsmasq.d/"+file)
                # Also delete all records for the domain name
        ...
        elif module == "dnsmasq":  # Synchronization resolution
            if fun == "update": # Update the configuration file from the database and reload the service
                ...
                if check_md5 == i['file_md5']:
                    ...
                    if sv_rt == 0:
                        ...
                        self.write("0") # success
                    else:
                        self.write("1") # Service restart failed
                else: # Md5 not match
                    self.write("2") # Verifying the configuration file failed
