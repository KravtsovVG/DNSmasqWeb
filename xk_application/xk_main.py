...
class TemplateRendering:
    ...
    def render_template(self, template_name, **kwargs):
        ...
        env = Environment(loader=FileSystemLoader(template_dirs),trim_blocks=True) # Trim_blocks Removes a lot of empty lines that are generated by html
...
class BaseHandler(RequestHandler, TemplateRendering):
    ...
    # Or the MD5 value of the file
    def get_md5(self,file):
        m = md5()
        # You need to read the contents of the file in binary format
        f = open(file, 'rb')
        m.update(f.read())
        f.close()
        return m.hexdigest()
...
# Module Permission Management Decorator
def Perm(method):
    @functools.wraps(method)
    def wrapper(self, *args, **kwargs):
        #### Access Control ####
        class_name = self.__class__.__name__  #Get the class name, in fact, to obtain the class name of the method to be used to facilitate a more precise authority to determine
        request_method = self.request.method  #Gets the request method name：GET|POST
        ...
        if permission:
            ...
            if is_admin == "no":
                self.write(''' <script type="text/javascript" >alert("Sorry，You do not have permission to operate！");</script> Sorry，You do not have permission to operate！''')
                return
        ###########
